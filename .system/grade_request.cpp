...
void exam::grade_request(bool i)
{
    if (!i)
    {
        std::cout << std::endl
                  ...
                  << "Please be " << LIME << "patient" << RESET << ", this " << LIME << "SHOULD" << RESET << " only take a few seconds..." << std::endl;
    ...
    // Log grading attempt
    {
        std::ofstream log_file("grading_history.log", std::ios::app);
        time_t now = time(0);
        char* dt = ctime(&now);
        std::string ex_name = current_ex ? current_ex->get_name() : "unknown";
        if (file_exists(".system/grading/passed"))
        {
            log_file << "[SUCCESS] " << dt << "Exercise: " << ex_name << std::endl;
            success_ex(0);
        }
        else
        {
            log_file << "[FAILURE] " << dt << "Exercise: " << ex_name << std::endl;
            std::cout << RED << ">>>>>>>>>> FAILURE <<<<<<<<<<<" << RESET << std::endl;
            sleep(1);
            std::cout << "You have failed the assignment." << std::endl;
            std::cout << YELLOW << "Possible reasons for failure:" << RESET << std::endl;
            std::cout << "- Compilation errors (check your code compiles without warnings)" << std::endl;
            std::cout << "- Failing test cases (review the requirements and test your code)" << std::endl;
            std::cout << "- Incorrect output format or edge cases not handled" << std::endl;
            std::cout << "- Missing files or wrong file names" << std::endl;
            std::cout << "- Permission issues or runtime errors" << std::endl;
            std::cout << "\nSuggestions:" << std::endl;
            std::cout << "- Read the traceback (if available) for more details." << std::endl;
            std::cout << "- Double-check the subject and requirements." << std::endl;
            std::cout << "- Ask for help if you are stuck!" << std::endl;
